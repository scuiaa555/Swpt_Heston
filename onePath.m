function disPayoff=onePath(m,n,i1,i2,f,H,K)
    ...
    for j=2:n*i1
        for p=2:T(j)/dt+1
            ...
            for k=2:j
                sig_rf=sig_rf-1/i1*f(k,p-1)/(1+1/i1*f(k,p-1))*gam_rf(k-1,(p-1)/12);
            end
            sig_D=1/i1*H(j,p-1)/(1+1/i1*H(j,p-1))*gam_H(j-1,(p-1)/12);
            f(j,p)=f(j,p-1)*exp(-V(p-1)*(gam_rf(j-1,(p-1)/12)'*sig_rf+0.5*norm(gam_rf(j-1,(p-1)/12))^2)*dt+sqrt(V(p-1))*(sqrt(1-rho^2)*gam_rf(j-1,(p-1)/12)'*dW(:,p-1)+rho*norm(gam_rf(j-1,(p-1)/12))*dZ(p-1)));
