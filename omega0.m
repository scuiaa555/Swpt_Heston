function [omg_f,omg_H,alp]=omega0(struc)
    ...
    for j=m*i1:n*i1-1
        ...
        for k=m*i1:j-1
            sum1=sum1+alp(k)*(f(k+1)+H(k+1)*exp(1/i1*H(k+1)/(1+1/i1*H(k+1))*I(k))+(1/i1*f(k+1))*H(k+1)*exp(1/i1*H(k+1)/(1+1/i1*H(k+1))*I(k)+J(k)));
        end
        for l=m:n-1
            if ((l+1)*i1-1)<j
        ...
        omg_f(j)=1/i1/(1+1/i1*f(j+1))*(sum1-sum2*s)+alp(j)+alp(j)/i1*H(j+1)*exp(1/i1*H(j+1)/(1+1/i1*H(j+1))*I(j)+J(j));
        ...
        omg_H(j)=alp(j)*exp(1/i1*H(j+1)/(1+1/i1*H(j+1))*I(j))*(1+1/i1*f(j+1)*exp(J(j)))*(1+H(j+1)*1/i1*I(j)/(1+1/i1*H(j+1))^2);
